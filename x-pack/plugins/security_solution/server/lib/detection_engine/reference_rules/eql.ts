...
interface EqlRuleTypeContext {
  [x: string]: unknown;
}

const eqlRuleType: AlertTypeWithExecutor<
  { eqlQuery: string; indexPatterns: string[] },
  EqlRuleTypeContext,
  PersistenceRuleTypeServices<EqlRuleTypeContext> & ExecutionLogServices
> = {
...
export const createEqlAlertType = (
  ...
  const withExecutionLog = withRuleExecutionLogFactory({
    ruleExecutionLogClient,
    logger,
  });

  return withExecutionLog(createPersistenceRuleType(eqlRuleType));
};
